...
export function AssetUtil(workspace) {
    ...
    function loadAudioAsset(assetId) {
        if (mLoadedAssets[assetId]) {
            Promise.resolve()
                .then(() => mLoadedAssets[assetId])
        } else {
            let asset = mModel.assets.find(a => a.id == assetId);
            if (!asset) { console.error("Invalid audio asset: " + assetId); return null; }

            let load = loadAudio(asset.filename)
                .then(buffer => {
                    mLoadedAssets[assetId] = buffer;
                    return mLoadedAssets[assetId];
                })
                .catch(e => { console.error('Failed to fetch audio: ' + assetId); console.error(e); return null; })

            // check the thumbnail asyncronously.
            load
                .then(buffer => buffer ? checkThumbnail(assetId, buffer, AssetTypes.AUDIO) : null);
    ...
    function loadAudio(filename) {
        mWorkspace.getFileAsDataURI(filename)
            .then(uri => new Promise((resolve, reject) => mAudioLoader.load(uri, resolve, null, reject)))
            .catch(e => {
                console.error(e);
    ...
    function loadThumbnail(itemId) {
        if (mLoadedThumbnails[itemId]) return Promise.resolve(mLoadedThumbnails[itemId]);
        else {
